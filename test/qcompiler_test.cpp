...
const std::string test_path = PMDK_PATH("qcompiler_tst");
...
TEST_CASE("Testing queries in compiled mode", "[qinterp]") {
  ...
  SECTION("Testing queries") {
    {
      auto res = qp.execute_query(query_proc::Compile, "NodeScan()");
    ...
    {
      auto res = qp.execute_query(query_proc::Compile,
    ...
    {
      auto res =
          qp.execute_query(query_proc::Compile,
